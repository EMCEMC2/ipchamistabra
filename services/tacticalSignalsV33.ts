/**
 * TACTICAL V3.3.1 - AI-ENHANCED SIGNAL SYSTEM WITH PATTERN LEARNING
 *
 * New in V3.3.1 (Reviewer Recommendations):
 * - OF veto now checks technical edge strength (techEdgeMinForVeto)
 * - Asset-specific session/weekend handling (assetType, disableWeekendPenalty, disableSessionPenalty)
 * - Crypto-optimized defaults (no weekend/session penalties)
 *
 * V3.3 Features:
...
export function detectStructureLevels(
  ...
  // CRITICAL FIX: Guard against empty or insufficient data
  ...
  // CRITICAL FIX: Ensure we have enough data for swing detection
  if (swingLookback < 5) {
    return levels;
  }
  ...
  // CRITICAL FIX: Guard against empty slices with spread operator
...
export function addToSignalHistory(
  ...
  // CRITICAL FIX: parseFloat can return NaN - validate before use
  const parsedEntry = parseFloat(signal.entryZone);
  const safeEntryPrice = Number.isFinite(parsedEntry) ? parsedEntry : 0;
...
export function calculatePatternSimilarity(
  ...
  // CRITICAL FIX: Guard against NaN in continuous similarities
  const safeTrendStrength = (val: number) => Number.isFinite(val) ? val : 0;
  const trendStrengthSim = 1 - Math.abs(safeTrendStrength(a.trendStrength) - safeTrendStrength(b.trendStrength));
  const volatilitySim = 1 - Math.abs(safeTrendStrength(a.volatilityPercentile) - safeTrendStrength(b.volatilityPercentile));
  const rsiSim = 1 - Math.abs(safeTrendStrength(a.rsiNormalized) - safeTrendStrength(b.rsiNormalized));
  const techEdgeSim = 1 - Math.min(1, Math.abs(safeTrendStrength(a.techEdge) - safeTrendStrength(b.techEdge)) / 3);
  const consensusSim = 1 - Math.abs(safeTrendStrength(a.consensusAgreement) - safeTrendStrength(b.consensusAgreement));

  score += Number.isFinite(trendStrengthSim) ? trendStrengthSim * 1.5 : 0;
  score += Number.isFinite(volatilitySim) ? volatilitySim * 1 : 0;
  score += Number.isFinite(rsiSim) ? rsiSim * 0.5 : 0;
  score += Number.isFinite(techEdgeSim) ? techEdgeSim * 2 : 0;
  score += Number.isFinite(consensusSim) ? consensusSim * 1 : 0;

  weights += 6; // Sum of continuous weights

  // CRITICAL FIX: Prevent division by zero - weights is always > 0 due to categorical matches
 ...
 * CRITICAL FIX: Added sample size decay to prevent runaway amplification
 */
...
function calculateOrderFlowScore(
  ...
  // CRITICAL FIX: Reduce staleness threshold from 30s to 10s for real-time data accuracy
  if (stats.lastUpdate && (Date.now() - stats.lastUpdate) > 10000) return defaultScore;
  ...
  // CVD Trend - derive from delta direction
  const cvdDelta = safeNumber(cvd.delta);
  const cvdCumulative = safeNumber(cvd.cumulativeDelta);
  const cvdTrend: 'BULLISH' | 'BEARISH' | 'NEUTRAL' =
    cvdDelta > 0 && cvdCumulative > 0 ? 'BULLISH' :
    cvdDelta < 0 && cvdCumulative < 0 ? 'BEARISH' : 'NEUTRAL';

  if (cvdTrend === 'BULLISH') bullScore += 1.2;
  else if (cvdTrend === 'BEARISH') bearScore += 1.2;

  // CVD Divergence - use delta for short-term direction
  ...
  // Absorption - use totalVolume as proxy for volume
  const priceMove = prevPrice > 0 ? Math.abs(price - prevPrice) / prevPrice : 0;
  const totalVol = safeNumber(stats.totalVolume, 1);
  // Consider high volume relative to typical threshold
  const hasHighVolume = totalVol > 1000000; // $1M in 60s is significant
...
export function calculateMultiTargets(
  entryPrice: number,
  stopPrice: number,
  direction: 'LONG' | 'SHORT',
  structure: MarketStructureV33,
...
function applySlippage(
  ...
  // CRITICAL FIX: Ensure minimum slippage floor of 1 bps even in low volatility
  const adjustedBps = Math.max(1, baseBps * Math.min(volRatio * 2.0, 3.0));
...
export function generateTacticalSignalV33(
  ...
  const emptyStructure: MarketStructureV33 = {
  ...
  const noSignal = (stage: string, reason: string, tech: TechnicalScore, of: OrderFlowScore | null, struct: MarketStructureV33, gates: QualityGateResult, cons: ConsensusResult | null, minScore: number, minEdge: number, chop: any, cool: any, pattern: any, reasoning: string[]): TacticalResultV33 => ({
    ...
    regime: struct.regime, reasoning: [...reasoning, `X ${reason}`],
  ...
  // OF Veto - V3.3.1: only apply if technical has meaningful edge
  if (config.useOrderFlow && orderFlow.direction !== 'NEUTRAL') {
    ...
    // If tech edge is weak but OF has strong signal, log it
    if (!techHasMeaningfulEdge && orderFlow.edge >= config.orderFlowVetoThreshold) {
      reasoning.push(`! Tech edge weak (${technical.edge.toFixed(2)}), OF signal: ${orderFlow.direction}`);
    }
  }
...
/**
 * Wrapper function to maintain backward compatibility with V2 interface
 * Converts V3.3.1 result to V2 format for existing consumers
 */
export function generateTacticalSignal(
  ...
  const v33Config: TacticalConfigV33 = {
    ...DEFAULT_CONFIG_V33,
    useOrderFlow: config.useOrderFlow ?? true,
    orderFlowWeight: config.orderFlowWeight ?? 0.3
  };

  // Generate V3.3.1 signal
  ...
  return {
    ...
    bullScore: result.technical.bullScore,
    bearScore: result.technical.bearScore,
    regime: result.structure.regime as 'LOW_VOL' | 'NORMAL' | 'HIGH_VOL',
    reasoning: result.reasoning,
    lastSignalBar: result.signal ? chartData.length - 1 : lastSignalBar
  };
}
