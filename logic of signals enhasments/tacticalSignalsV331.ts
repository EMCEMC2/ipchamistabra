...
export const DEFAULT_CONFIG_V33: TacticalConfigV33 = {
  ...
  techEdgeMinForVeto: 0.5,        // NEW V3.3.1: Only veto if tech has real preference
...
function runQualityGates(ctx: GateContext): QualityGateResult {
  ...
  // SOFT GATES (with asset-specific toggles - V3.3.1)
...
export function generateTacticalSignalV33(
  ...
  if (config.useOrderFlow && orderFlow.direction !== 'NEUTRAL') {
    ...
    if (!techHasMeaningfulEdge && orderFlow.edge >= config.orderFlowVetoThreshold) {
      reasoning.push(`⚠️ Tech edge weak (${technical.edge.toFixed(2)}), OF signal: ${orderFlow.direction}`);
    }
  }
