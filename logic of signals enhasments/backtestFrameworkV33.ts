...
export class BacktestEngine {
  ...
  private closePosition(
    ...
    const risk = Math.abs(position.entryPrice - parseFloat(position.signal.invalidation));
    ...
    const closedTrade: ClosedTrade = {
      ...
      pnlPercent: (position.realizedPnL / position.positionSize) * 100,
      rMultiple,
  ...
  private calculateResults(): BacktestResults {
    ...
    const winRateByTrend = this.calculateWinRateByCategory(trades, t => t.signal.patternFingerprint.trendType);
    const winRateByDirection = this.calculateWinRateByCategory(trades, t => t.signal.type);
    const winRateByCvdDivergence = this.calculateWinRateByCategory(trades, t => t.signal.patternFingerprint.cvdDivergence);
...
export { BacktestEngine, formatBacktestResults };
