...
export const ExecutionPanel: React.FC<ExecutionPanelProps> = ({ currentPrice, balance, onExecute, initialValues }) => {
  ...
  useEffect(() => {
    ...
    if (orderType === 'MARKET' && !stopLoss && !initialValues) {
        const atrMock = currentPrice * 0.015; 
        if (side === 'LONG') {
            setStopLoss((currentPrice - atrMock).toFixed(2));
            setTakeProfit((currentPrice + (atrMock * 2)).toFixed(2));
        } else {
            setStopLoss((currentPrice + atrMock).toFixed(2));
            setTakeProfit((currentPrice - (atrMock * 2)).toFixed(2));
        }
    ...
    <div className="h-full flex flex-col bg-terminal-card border border-terminal-border rounded-lg p-5 shadow-xl">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4 p-1 bg-terminal-bg rounded-lg border border-terminal-border">
          {['MARKET', 'LIMIT'].map(t => (
              <button
                ...
                className={`text-xs py-2 rounded font-mono font-bold transition-all ${
                    orderType === t 
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
          <button
            ...
            className={`py-4 rounded-lg font-mono font-bold text-sm border transition-all flex flex-col items-center gap-1 ${
                side === 'LONG' 
      ...
      <button 
        ...
        className={`w-full py-4 mt-4 rounded-lg font-mono font-bold text-sm tracking-wider transition-all hover:brightness-110 hover:scale-[1.02] shadow-lg flex items-center justify-center gap-2 ${
            side === 'LONG' 
